    ...
    internal class FileUtil {
        ...
        public static bool Write(string filePath, string content) {
            ...
            using (FileStream fs = new FileStream(filePath, FileMode.OpenOrCreate)) {
                lock (fs)//锁住流
                {
                    ...
                    fs.Write(buffer, SystemConstant.FILE_SAVA_OFFSET, buffer.Length);
                    return true;
                }
            }
        }
        ...
        private static bool CopyDir(DirectoryInfo fromDir, string toDir, string rootDir) {
            ...
            foreach (FileInfo f in fromDir.GetFiles()) {
                ...
                string newDir = filePath.Substring(SystemConstant.NUMERIC_TYPE_ZERO, filePath.LastIndexOf("\\"));
                CreateDir(newDir);
        ...
        public static bool DeleteDir(string dir, bool onlyDir) {
            ...
            if (dirInfo.GetFiles().Length == SystemConstant.NUMERIC_TYPE_ZERO && dirInfo.GetDirectories().Length == SystemConstant.NUMERIC_TYPE_ZERO) {
        ...
        /// <returns>查询状态，true 存在；false 不存在</returns>
